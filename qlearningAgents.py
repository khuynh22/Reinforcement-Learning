...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        for action in legalActs:
            t[action] = self.getQValue(state, action)
        return t[t.argMax()]
    ...
    def update(self, state, action, nextState, reward):
        ...
        rw = self.alpha * reward
        if not nextState:
            self.q_values[(state, action)] = old + rw
        else:
            nextState_part = self.alpha * \
                self.discount * self.getValue(nextState)
            self.q_values[(state, action)] = old + \
                rw + nextState_part
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        """
        d = (reward + self.discount * self.getValue(nextState)) - \
            self.getQValue(state, action)
        features = self.featExtractor.getFeatures(state, action)
        for i in features:
            self.weights[i] = self.weights[i] + self.alpha * d * features[i]
